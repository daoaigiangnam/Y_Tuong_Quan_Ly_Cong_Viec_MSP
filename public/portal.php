...
$canDashboard = rbac_authorize($user, (array)($user['permissions'] ?? []), 'dashboard.view', $portal) === 'ALLOW';
$canTicketRead = rbac_authorize($user, (array)($user['permissions'] ?? []), 'ticket.read', $portal) === 'ALLOW';
$canTicketCreate = rbac_authorize($user, (array)($user['permissions'] ?? []), 'ticket.create', $portal) === 'ALLOW';
$canContractRead = rbac_authorize($user, (array)($user['permissions'] ?? []), 'contract.read', $portal) === 'ALLOW';
...
<a class="navbar-brand fw-bold" href="?page=portal&portal=<?=e($portal)?>">MSP ITSM</a>
...
<a href="?page=portal&portal=<?=e($portal)?>"><i class="bi bi-grid"></i> Dashboard</a>
